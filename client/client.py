...
def display_image_with_bounding_boxes(dicom, bounding_boxes, width, height):
    window = tk.Tk()
...
def generate_image_request(dicom_file):
    ...
    for i in range(0, len(pixel_data), chunk_size):
        ...
        yield comms.ImageRequest(
            file_type='',  # DICOM format doesn't require specifying a file type
            width=width,
            height=height,
            image=chunk
        )
...
def run_client():
    ...
    display_image_with_bounding_boxes(file, bounding_boxes, 800, 800)
